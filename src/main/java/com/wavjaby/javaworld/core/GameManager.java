...
import com.wavjaby.javaworld.core.entity.EntityData;
import com.wavjaby.javaworld.core.jwblocks.Grass;
import com.wavjaby.javaworld.core.jwblocks.Stone;
import com.wavjaby.javaworld.core.jwentities.Self;
import com.wavjaby.javaworld.core.jwentities.Tree;
import com.wavjaby.javaworld.data.PlayerConsoleOutput;
import com.wavjaby.javaworld.data.WorldEntityUpdate;
import com.wavjaby.javaworld.server.ClientHandler;
import com.wavjaby.javaworld.server.CompiledResult;
import com.wavjaby.javaworld.util.CustomThreadFactory;
...
import static com.wavjaby.javaworld.adapter.block.Chunk.CHUNK_SIZE;
import static com.wavjaby.javaworld.core.EntityUpdate.UpdateType.UPDATE;
import static com.wavjaby.javaworld.util.ExceptionToString.exceptionToErrorPrintStream;
...
public class GameManager {
        ...
        try {
            for (ClientHandler client : clients.values()) {
                ...
                if (player.isMoving()) {
                    ...
                    if (distanceLeft < player.getVelocity().length()) {
                        // Moving finish
                        entitySetPosition(player, player.getCurrentDest(), movedEntities);
                        player.setVelocityZero();
                        player.setCurrentDest(null);
                    } else
                        entitySetPosition(player, player.getPosition().add(player.getVelocity().mul(1d / TICK)), movedEntities);
                }
            }
        } catch (Exception e) {
    ...
    private void entitySetPosition(Entity entity, Vec2 position, Map<Integer, EntityUpdate> movedEntities) {
        ...
        movedEntities.computeIfAbsent(entity.getSerial(), (k) -> new EntityUpdate(UPDATE, entity));
    }
